...
## 2026-03-13 - CI Fixing: Pydantic V2 ValidationError Matching and Deque Clears

**Learning:** During CI execution, two errors surfaced. First, `ea_http.pending_signals` is defined as a `deque`, but tests were assigning it a `list` `[]` during cleanup, leading to `AttributeError: 'list' object has no attribute 'popleft'`. Second, Pydantic V2 raises a single combined multi-line `ValidationError` string containing all field errors instead of separate exceptions. Older tests using strict regex matching (`match="SECRET_KEY must be changed"`) failed because they did not account for other simultaneous validation errors (like `EXNESS_LOGIN must be changed`).

**Action:** For collection clears in tests, I updated `.clear()` instead of type-changing reassignments (`.clear()` works for both lists and deques). For Pydantic tests, I updated regex match strings to use wildcards (`match=".*EXPECTED_TEXT.*"`) to successfully match specific field errors inside the combined multi-line error string. Additionally, I added `clear=True` to `patch.dict(os.environ, env_vars, clear=True)` for settings testing to ensure default fallback behaviors are accurately tested without contamination from the host environment.
