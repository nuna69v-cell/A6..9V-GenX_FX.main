    ...
    for repo_name in mouy_repos:
        if repo_name in leng_repos:
            ...
            if not os.path.exists(mouy_repo_path) or not os.path.exists(leng_repo_path):
                print(
                    f"  ⚠️ Skipping {repo_name} - Not cloned locally in both accounts."
                )
                continue
            ...
            if check_uncommitted_changes(mouy_repo_path):
                print(
                    f"  ⚠️ Warning: Uncommitted changes in {mouy_repo_path}. Commit before syncing."
                )
                continue

            if dry_run:
                print(
                    f"  [DRY RUN] Would copy updates from {mouy_repo_path} to {leng_repo_path}"
                )
            else:
                ...
                print(
                    f"  [Action Needed] Add robust sync logic here depending on desired Git history integration."
                )
                print(
                    f"  Example: Add {mouy_repo_path} as a remote to {leng_repo_path} and pull."
                )

                # Adding mouyleng remote to lengkundee
                remotes = run_command(
                    "git remote -v", cwd=leng_repo_path, hide_output=True
                )
                if "mouyleng_remote" not in (remotes or ""):
                    print(f"  🔗 Adding remote 'mouyleng_remote' to {leng_repo_path}")
                    run_command(
                        f"git remote add mouyleng_remote {mouy_repo_path}",
                        cwd=leng_repo_path,
                        hide_output=True,
                    )
                ...
                run_command(
                    "git fetch mouyleng_remote", cwd=leng_repo_path, hide_output=True
                )

                # Attempting merge (might require manual conflict resolution)
                print(f"  🔀 Merging updates...")
                merge_result = run_command(
                    f"git merge mouyleng_remote/main --allow-unrelated-histories -m 'Auto-sync from mouyleng172'",
                    cwd=leng_repo_path,
                    hide_output=True,
                )
                if merge_result is None:
                    print(
                        f"  ⚠️ Merge conflict or error in {repo_name}. Manual resolution required."
                    )
                else:
