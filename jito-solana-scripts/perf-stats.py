...
for stage in stages_data.keys():
    ...
    print(
        "stage: {} max: {:,.2f} min: {:.2f} count: {} total: {} mean: {:,.2f} average: {:,.2f} 80%: {:,.2f}".format(
            stage,
    ...
    if data_len >= num:
        ...
        print("")
    print(
        "    max_ts: {} min_ts: {}".format(
            stages_data[stage]["max_speed_ts"], stages_data[stage]["min_speed_ts"]
        )
    )
    print("\n")
