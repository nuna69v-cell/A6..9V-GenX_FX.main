...
//--- Trading settings
input int      MagicNumber      = 169926;   // Magic number for orders
...
int OnInit()
{
   ...
   Print("=== GenX FX EA (Exness) v3.0 starting ===");
...
void OnDeinit(const int reason)
{
   ...
   Print("=== GenX FX EA (Exness) stopped ===");
}
...
{
   string body = "{\"eaName\":\"GenX-Exness-EA\","
...
void PollAndExecuteSignals()
{
   ...
   // Parse each signal – simple JSON parsing
   int pos = 0;
   while(true)
   {
...
void ProcessSignalJSON(string json)
{
   ...
   // If symbol is empty or "any", trade current chart symbol
   ...
   {
      Print("Max positions reached (", MaxOpenPositions, "). Signal skipped.");
      return;
   }
   ...
   {
      Print("Spread too high (", SymbolInfoInteger(symbol, SYMBOL_SPREAD), " pts). Signal skipped.");
      return;
   }
...
//+------------------------------------------------------------------+
//| HTTP helpers using MT5 WebRequest                                |
//+------------------------------------------------------------------+
